...
## About the PLUs
The {avocadoo} package consists of 6 different [PLUs](https://en.wikipedia.org/wiki/Price_look-up_code):
...
The `hass` dataset provides a weekly sales summary of Hass Avocado sales in the contiguous US (subdivided by region and select cites/sub-regions within each 'parent' region):
...
The source of the data for this package is from the Hass Avocado Board. However, the data are known to come from a 3rd party source and is the best available information at the time it was provided to the Hass Avocado Board. There are many instances where adding up individual columns will not equal the total columns that are provided. No explanation is provided by the Hass Avocado Board about this discrepancy (or at least I couldn't find any information).
