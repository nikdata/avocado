...
The `hass_region` dataset focuses on weekly US Hass Avocado sales at the region level and consist of 8 regions (for convenience, the states within each region are also listed - but are not available within the dataset since sales are not broken down by state):
...
The `hass_region` dataset fields:
